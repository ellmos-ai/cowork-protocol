...
| Browser Companion works on a page without Cowork or WebMCP | Browser Companion protocol/build tests; Chrome for Testing 152 `npm run smoke:companion` | Self-contained MV3 artifact, browser-wide attachment, real semantic/crop callbacks and trusted-click host path accepted; store-ready `activeTab` permission minimization and a connected extension model host remain open |
...
| Browser extension is Native-first and keeps UI outside the page DOM | MV3 main/isolated-world plus Side Panel build tests; Chrome 152 `npm run smoke:companion-native` and `npm run smoke:companion` | Native FormBuilder exposes nine tools with `fallbackActive: false`; no-WebMCP fixture uses the bounded fallback; both report `pageUiInjected: false` |
| Exact session state can join a persistent local Companion | Session Authority, Context Manager, Model Gateway, Companion Link and desktop-host tests; Chrome 152 `npm run smoke:surface` | Two contiguous authority deltas claim Desktop surface and renewable model seat; context persists; a separate app window sends one serialized model turn while Embed disables its model input |
...
| Rendered text contrast covers the collaboration state matrix | `visual-theme.test.js`; `pixel-contrast-smoke.test.js`; `npm run smoke:contrast` | Chrome 152 audited 699 visible text items across 10 exact states: 0 unsupported, 0 failures, unrounded minimum 4.565644512773976:1; Listening is a visual fake-recognition boundary and makes no audio claim |
...
Those URLs currently reflect `main`; the post-audit Companion increment in this
working tree still requires its final commit, push and Pages readback.
