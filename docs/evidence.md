...
| Browser Companion works on a page without Cowork or WebMCP | Browser Companion protocol/build tests; two consecutive Chrome for Testing 152 `npm run smoke:companion` passes | Self-contained MV3 artifact, browser-wide attachment, real semantic/crop callbacks and trusted-click host path accepted; store-ready `activeTab` permission minimization and a connected extension model host remain open |
...
Local release preparation: a clean clone of connected-model commit `1753505c38310dda007cc9e9de40981fb67a97cf` passed `npm ci`, 152/152 Node tests, 12/12 character evals, 8/8 proof steps, the 468-character deterministic model-host fixture and a 502-character provider-backed local Qwen run with `preferredModelClaim: true`, `connectedModelClaim: true`, `externalModelClaim: false`, no browser credentials and a trusted-click receipt. The same clone passed native Chrome 152 discovery of nine Cowork tools plus local helper/WebMCP-reply/browser-host/200%-zoom smokes, the 21-control accessibility-tree/Tab/390px smoke, the 10-state rendered contrast smoke with 649 audited text items, 65 tracked and 13 built syntax checks, a 19-file Pages build, 0 secret findings, 0 dependency vulnerabilities and a clean Git readback. This commit is local and has not been pushed during this gate.
