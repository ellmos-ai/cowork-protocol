...
| Nine Native WebMCP tools register and clean up | `packages/native-webmcp/test/registration.test.js`; `npm run smoke:webmcp` | Contract fake plus Chrome 152 discovery and eight focus/context/offer/change/feedback/conversation invocations proven; re-run on the installed branded Google Chrome 152.0.7977.77 (`COWORK_CHROME_PATH`, features `WebMCP,WebMCPTesting`), nine tools and every report claim true, so "Google Chrome with WebMCP enabled" is measured literally and not only on a Chrome for Testing build; connected-agent journey open |
...
| A human who has stepped away is not required to point (K3) | `packages/core/test/presence.test.js`; `apps/desktop-companion/test/host.test.js`; `npm run smoke:webmcp`; `npm run smoke:builder` | Both handover paths previously refused without a focused field, so the away path failed exactly when it was needed and the model was never seen working alone. Stepping away now always grants over the whole canvas, and an empty job description names the default for that scope instead of blocking. Chrome 152 (`soloWithoutPointer`), after clearing the lens and stepping away: grant over 4 targets, 4 of 4 fields filled with no click, 4 verified `during Agent Solo` receipts, 4 fields highlighted on return, and `cowork_read_focus` still refusing - correct, because nobody is pointing; the targets come from the grant, which `cowork_read_presence` carries. `smoke:builder` clears the Studio lens before stepping away and still spends the whole six-draft budget. Node: the Companion mints a 4-target, 6-call grant from the page's published target list, refuses only a page reporting no targets at all, and delivers a model answer inside an away whole-form grant as `cowork_execute_solo` while a target outside the grant still waits for a click. Native tool count unchanged at 9 |
| The cockpit acts on the live session, not on a session nobody speaks to | `apps/desktop-companion/test/host.test.js` | Measured on the user's own Companion (store at %LOCALAPPDATA%\Cowork Protocol): six link sessions for one page, four of them with no page contact, and the cockpit's clicks landed on one of those while the live session stayed on paused - which reads as "it springs back". The UI state now lists most recent page contact first and never-contacted last, and a page that links again drops its own contactless leftover. Both tests drive a real store restart, which is how the twins appeared |
| One session identity across both canvases, and a Studio grant that can act | `apps/formbuilder-showcase/test/builder-cowork-ui.test.js`; `npm run smoke:webmcp` | The Studio bridge called itself `formbuilder-showcase-builder` while every store session was `formbuilder-showcase`, and `authorityState()` published the sample form's focus only - so a Companion found no focus while the human pointed at the Studio and its seat click had nothing to grant about. The page now publishes the live focus plus the active canvas's targets and capabilities. Under a canvas-scoped grant `cowork_execute_solo` adds a Studio field with no offer and no click: the Studio adopts the human's grant instead of issuing one, so scope, budget and expiry are unchanged. Chrome (`studioSoloAddedFieldWithoutClick`): the field landed on the canvas and no offer was left waiting. Node: an adopted grant copies the budget rather than widening it, a spent budget refuses, and an empty label and an expired grant both fail closed |
| The Companion's talk button says why it could not listen | `apps/desktop-companion/test/cockpit-ui.test.js` | Every failure read "No speech turn created.", so a blocked microphone, a missing one and an unreachable speech service were indistinguishable. Each reported cause now names its remedy; the dictation language follows the window; a second start no longer throws into nothing. No microphone is exercised - the test asserts the messages, not real capture |
| The model's figure never cycles into "away" (K7) | `apps/formbuilder-showcase/test/session.test.js`; `npm run smoke:webmcp` | For a model, away means no seat connected at all, so pressing the seat into it read as a lost connection and the human had to reach for the other figure to recover - while the Companion cockpit has only ever offered paused and active. The page's model figure now cycles three states and says "Model paused - click again to resume" when it parks. Chrome (`modelSeatStates`): four trusted presses gave standby, here-executing, here-advising, standby - no away. The human's figure keeps all four states; the browser extension's own cycle is untouched |
| The cockpit never claims a model whose seat it does not hold | `apps/desktop-companion/test/host.test.js`; `apps/desktop-companion/test/cockpit-ui.test.js` | Measured on the user's own Companion: header "Restored session - page not linked", cockpit "Model is advising - qwen3.8:27b-mlx", and a typed turn came back MODEL_SEAT_NOT_OWNED. Cause: the seat lease runs 30 minutes, a session waiting in the store outlives it, and `renewModelSeat` refuses by design to extend a lapsed lease. Decision: a restored session takes its OWN lapsed seat back with `claimModelSeat`, which the protocol already refuses while anyone still holds one (MODEL_SEAT_OCCUPIED) - so a provider that took over keeps it. Where the seat is not ours the cockpit shows the model as away, names it "no seat for this session", says to link the page, and disables input, send and talk instead of sending a turn that fails. `ownsModelSeat()` is exported from `packages/model-gateway` so the gateway's refusal and the cockpit's display read the same condition. Node: lapsed seat reclaimed and the turn goes through; a live provider seat left untouched and the turn refused |
...
- **Push to talk keeps listening, and the space bar holds it open** (D9). The recognition session ended at the first natural pause, so a held button fell back to off mid-sentence. Two collecting modes now exist beside the single press: `Keep listening`, and the space bar held down. Both run the recognition with `continuous = true` and write finished phrases into the conversation input without sending, so a dictated paragraph is one turn the human sends rather than a turn per phrase. The state machine is shared, not duplicated: `createHoldToTalk` and `readFinalTranscript` in `packages/reference-ui/src/index.js` drive both the page panel and the Companion window, and both surfaces gained the same `Keep listening` checkbox. `node --test` 528 pass (522 before, +6 for the state machine: hold starts once and a key repeat is not a second press; a release nobody pressed, another key and a Ctrl-chord all do nothing; the space bar stays a space in an input, textarea, button or contenteditable; a window that loses focus mid-hold closes the microphone; continuous mode reads only new final phrases from `resultIndex` and never an interim one; a one-shot result without `isFinal` still reads, so a single press keeps working). Not evidenced: a real microphone. The gap in "Explicitly not yet evidenced" below still stands - this is measured logic, not measured audio.
- **The AX baseline was re-measured, not derived** (D9, then D8). `CURRENT_INTERACTIVE_CONTROL_COUNT` moved 34 -> 35 for `Keep listening` and 35 -> 36 for `Chat window`, both in the default view. `npm run smoke:accessibility` reported 36 interactive, 36 reachable and 36 focus-visible controls, with a 36-long unique tab sequence, and exits 0; `npm run smoke:webmcp` exits 0 against the same number.
- **Role and Conversation as a chat window** (D8). The two sections a person converses through were the smallest part of the panel. The smaller of the two available solutions was taken: no second Picture-in-Picture window and no node juggling, because the existing `Detach` already moves the whole panel into a PiP window at full width and Role and Conversation are already adjacent in it. `Chat window` opens that same window under one layout class - other sections hidden, transcript given the remaining height with its own scrollbar, input taller - so session, model seat, handlers and fold states are untouched by construction. `npm run smoke:surface` exits 0 with eight new named expectations, including: the detached body carries the chat layout, the visible sections are exactly `work-mode-heading,audio-heading`, the transcript renders at 100px or more (against the 2.2rem it gets docked) and scrolls, the session id is unchanged, and closing the window docks the panel back into the page.
