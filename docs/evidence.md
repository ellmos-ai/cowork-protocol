...
| Seven Native WebMCP tools register and clean up | `packages/native-webmcp/test/registration.test.js`; `npm run smoke:webmcp` | Contract fake plus Chrome 152 discovery and six focus/context/offer/change/feedback invocations proven; connected-agent journey open |
...
| Public-preview tree is reproducible | clean clone of local `release/public-preview` at `64fa1eb` | `npm ci`, 93/93 tests, 11/11 character evals, 7/7 proof steps, full native Chrome 152 human-loop smoke, 16-file Pages build, 44 syntax checks, 0 vulnerabilities, 0 secret findings and clean scoped history |
