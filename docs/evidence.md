...
- **The AX baseline was re-measured, not derived** (D9). `CURRENT_INTERACTIVE_CONTROL_COUNT` moved 34 -> 35 for the new `Keep listening` control, which sits in the default view. `npm run smoke:accessibility` and `npm run smoke:webmcp` both exit 0 against that number.
