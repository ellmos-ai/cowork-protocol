...
`npm run smoke:companion` is the separate real-extension acceptance. It builds `dist-browser-companion`, starts Chrome for Testing 152 in a fresh profile, loads the unpacked Manifest V3 artifact and explicitly disables WebMCP on the fixture page. The extension must begin off, toggle on, expose a stable pointer target, return exactly bounded 350- and 1,200-character semantic tiers, capture and immediately crop a real 400×400 PNG, deliver that crop once only inside the isolated extension host, keep a visible exact-value offer inert, accept a trusted browser click, verify the new field value and toggle off again. The report sets `browserCompanionClaim`, `visualCaptureClaim` and `visualDeliveryOneShot` true while keeping model-client, external-model, host-token and full-page-delivery claims false.
