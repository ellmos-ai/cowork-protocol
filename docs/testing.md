...
- Desktop Companion host, UI, app-window and Windows tray tests covering loopback/origin gates, persisted context, shared turns and green/yellow/red presence projection;
...
`npm run smoke:companion` is the real no-WebMCP extension acceptance. It builds `dist-browser-companion`, starts Chrome for Testing 152 in a fresh profile, loads the unpacked Manifest V3 artifact and explicitly disables WebMCP on the fixture page. The extension must begin off, toggle on, expose a stable pointer target, return exactly bounded 350- and 1,200-character semantic tiers, capture and immediately crop a real 400×400 PNG, deliver that crop once only inside the isolated extension host, keep a Side Panel exact-value offer inert, accept a trusted browser click, verify the new field value and toggle off again. The page must contain no injected Cowork UI root. The report sets `browserCompanionClaim`, `sidePanelSurfaceClaim`, `visualCaptureClaim` and `visualDeliveryOneShot` true and `pageUiInjected` false while keeping model-client, external-model, host-token and full-page-delivery claims false.

`npm run smoke:companion-native` is the complementary Native-first acceptance. It enables WebMCP on FormBuilder and loads the same extension. A minimal main-world bridge must discover nine native Cowork tools; the isolated extension runtime must report `mode: native-cowork`, return the page's native focus packet, keep `fallbackActive: false` and insert no visual root into the page.

`npm run smoke:surface` exercises the Cowork-owned continuity path without an extension. Chrome moves FormBuilder's selected reference UI from Embed to Document Picture-in-Picture and back, grants the user-mediated loopback permission, sends one exact snapshot plus compact context to the Companion, and applies the two contiguous authority deltas for Desktop surface and model seat. It then opens a separate 430×760 Companion window, verifies the shared reference identity and three audio controls, submits one deterministic turn through the serialized Model Gateway, observes human plus assistant context, and confirms that the embedded model input is disabled after handoff.
...
- Store-ready permission minimization for the Browser Companion. The accepted prototype uses a broad page match but stays inert until toggled; a distribution build should move to `activeTab` plus on-demand injection and repeat the same browser acceptance.
