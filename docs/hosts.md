...
flowchart TB
  ...
  S --> D["Desktop Companion<br/>app window on the OS<br/>session authority · model seat · MCP server · tray"]
  ...
  A2["Local agents over MCP<br/>(Claude Code, Codex CLI: not yet measured)"] -. tools/call .-> D
...
| Desktop Companion | An app window on the operating system (`apps/desktop-companion`) | Freedom. Session authority outside the browser, a model seat whose endpoint and key never enter a page, presence in the tray, a filtered Computer Use fallback, and an MCP server so local agents (Claude Code, Codex CLI, comparable tools) can use the Companion as a tool. | It is not a browser agent and does not discover sites on its own; a page has to link to it. |
